...
本项目即 git 仓库 **`codelm`**（本地 `画布项目/codelm/`，远端 `github.com/ricroad/codelm`）。**仓库根就是 App**：
```
codelm/                  # = 本 App 仓库根
...
```
外部依赖（在仓库之外、**不纳入本 repo**）：`../frontend-repo`（被导览的代码库 + 其 `.understand-anything/knowledge-graph.json`）。从 codelm 根看 `../frontend-repo` 即 `画布项目/frontend-repo`，与 §14 默认路径一致。
...
4. 图谱/repo 根路径：默认 `../frontend-repo`，设置里可改（为日后导览别的库留口）。
